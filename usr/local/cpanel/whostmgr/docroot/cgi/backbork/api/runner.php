<?php
/**
 * BackBork KISS - Background Job Runner
 * 
 * This script is executed as a background process to run backup/restore jobs.
 * It reads a job file, executes the appropriate operation, and cleans up.
 * 
 * Usage: php runner.php /path/to/job.job
 * 
 * The job file is JSON containing:
 * - type: 'backup' or 'restore'
 * - backup_id/restore_id: The operation ID
 * - accounts/account: The account(s) to process
 * - destination: The destination ID
 * - user: The user who initiated the job
 * - options: Additional options (for restore)
 *

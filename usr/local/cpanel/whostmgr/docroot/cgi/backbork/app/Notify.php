 ...
 * 
 * Simple, robust notification system using text templates.
 * Sends email and Slack notifications for backup/restore/system events.
 *
 * Templates (in templates/notifications/):
 * - job.txt: For backup/restore events (started/completed/failed)
 * - system.txt: For system alerts (cron, queue, pruning, test)
 *
 * To add new events, just add an entry to the $events array.
 *
    ...
    public function sendSlack($webhookUrl, $message) {
        ...
        $payload = [
            ...
            'attachments' => [[
                ...
                'pretext'  => ($message['emoji'] ?? '') . ' ' . $message['subject'],
                'text'     => $message['body'],
                'footer'   => 'BackBork KISS | ' . gethostname(),
                'ts'       => time()
            ]]
        ];
